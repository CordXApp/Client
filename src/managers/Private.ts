...
import { ISlashCommand } from '../types/utils.interface'
...
export default class PrivateManager {
    ...
    public category(category: string): Collection<string, ISlashCommand> {
        return this.private.filter((cmd: ISlashCommand) => cmd.props.category === category)
    }
        ...
        readdirSync(dir).forEach(async (subDir: string): Promise<void> => {
          ...
          for (const file of commands) {
            ...
            if (command.props.name && typeof (command.props.name) === 'string') {
              if (this.private.get(command.props.name)) return this.logs.error(`${command.props.name} is already a used name`);
