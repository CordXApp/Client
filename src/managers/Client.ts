...
export class CustomDomains {
    ...
    public async checkTxtRecord(): Promise<boolean> {
        return new Promise((resolve, reject) => {
            ...
            if (!this.txtName)
                return reject(new Error("No TXT record name provided."))
            if (!this.domain) return reject(new Error("No domain provided."))
            if (!ipRegex.test(this.domain))
                return reject(new Error("IP Addresses are not supported."))

            const parts = this.domain.split(".")
            const main = parts.slice(-2).join(".")

            if (main === "localhost")
                return reject(new Error("Localhost is not supported."))

            DNS.resolveTxt(main, (err, records) => {
                if (err) return reject(err)
                if (!records) return reject(new Error("No TXT records found."))

                const hasTxtRecord = records.some((record) =>
                    record.includes(this.txtName!),
                )
