...
module.exports.websiteMonitor = async ({ client, domain, interval, retries, logChannelId }) => {
    
    if (!client) return console.log('[ERROR - client]: please provide a valid monitor client');
    if (!domain) return console.log('[ERROR - domain]: please provide a valid monitor domain');
    if (!interval) return console.log('[ERROR - interval]: please provide a valid monitor interval');
    if (!retries) return console.log('[ERROR - retries]: please provide a valid number of monitor retries');
    if (!logChannelId) return console.log('[ERROR - logChannelId]: please provide a discord channel id');

    if (interval < 900000) return console.log('[ERROR]: monitor interval should be greater then 900000');
    if (retries > 3) return console.log('[ERROR]: maximum amount of retries should be 3 or less');
    ...
    if (!chan) return console.log('[ERROR - logChannelId]: please provide a valid discord channel id')
