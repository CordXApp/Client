...
module.exports.startProdSiteMonitor = async ({ client }) => {
    ...
    const chan = await guild.channels.cache.find((c) => c.id == '1148439450569953280');
    ...
    uptime.on('outage', async (outage) => {
        ...
        if (outage.statusCode || !outage.includes('monitor failure')) await chan.send({
            embeds: [
                new client.Gateway.EmbedBuilder()
                .setTitle(`Status of: ${uptime.infos.url}`)
                .setColor(client.colors.base)
    ...
    uptime.on('error', async (error) => {
        ...
        await chan.send({
            embeds: [
                ...
                .setColor(client.colors.base)
                .setThumbnail(client.logo)
                .setDescription('Something went wrong here, check the console for details. Monitor will be stopped now!')
                .setTimestamp()
                .setFooter({
                    text: client.footer,
                    iconURL: client.logo
