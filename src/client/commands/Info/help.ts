...
export default class About extends SlashBase {
  ...
  public async execute(client: CordX, interaction: ChatInputCommandInteraction<CacheType>): Promise<any> {
    ...
    if (cmd && !client.commands.get(cmd)) return interaction.reply({ content: 'That command does not exist!', ephemeral: true });
        ...
        embeds: [
            new client.Embeds({
                ...
                fields: [
                    {
                        name: 'Info Commands',
                        value: `${client.commands.category('Info').map((cmd: any) => `\`${cmd.props.name}\``).join(' ')}`,
                        inline: true
                    }
                ]
            })
